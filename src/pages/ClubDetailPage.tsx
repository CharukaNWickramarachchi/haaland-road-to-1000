    ...
    <section className="mx-auto max-w-[1440px] px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
      ...
      <header className="mt-10 grid gap-10 border-b border-white/8 pb-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
        <div>
          ...
          <h1 className="mt-4 max-w-4xl text-5xl font-black tracking-[-0.055em] uppercase sm:text-7xl">
            {club}
          </h1>

          <p className="mt-5 max-w-xl text-base leading-7 text-white/45">
            Complete scoring history derived from the canonical
      ...
      <div className="grid gap-px bg-white/8 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <HeroStat
          icon={<Target size={17} />}
          label="Opponents"
          value={analytics.opponents}
        />

        <HeroStat
          icon={<MapPin size={17} />}
          label="Home goals"
          value={analytics.homeGoals}
        />

        <HeroStat
          icon={<MapPin size={17} />}
          label="Away goals"
          value={analytics.awayGoals}
        />
      </div>
...
function GoalMilestone({
  ...
  return (
    <Link
      to={`/goals/${goalNumber}`}
      className="group border border-white/8 p-6 transition hover:border-[#6CABDD]/30"
    >
